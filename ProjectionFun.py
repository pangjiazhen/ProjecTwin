...
def show_warped(img, warped, c_src, c_dst):
    ...
    axs[0].scatter(c_src[:, 0] * img.shape[1], c_src[:, 1] * img.shape[0], marker='+', color='white')
    axs[0].set_facecolor('white')  # Set the background color to white
    axs[1].imshow(warped[..., ::-1], origin='upper')
    axs[1].scatter(c_dst[:, 0] * warped.shape[1], c_dst[:, 1] * warped.shape[0], marker='+', color='white')
    axs[1].set_facecolor('white')  # Set the background color to white
    plt.show()
