...
import generation_mengpi as gm
...
values = check
# ax.scatter(mengpi.samplingcloud[:, 0], mengpi.samplingcloud[:, 1], mengpi.samplingcloud[:, 2], c='b', marker='.')
scatter = ax.scatter(x, y, z, c=values, cmap='jet')
fig.colorbar(scatter)  # 添加颜色条
# for i, value in enumerate(check):
#     if value>0.01:
#         ax.scatter(mengpi.samplingcloud[i, 0], mengpi.samplingcloud[i, 1], mengpi.samplingcloud[i, 2], c='g', marker='.')
ax.set_xlabel('X')
ax.set_ylabel('Y')
ax.set_zlabel('Z')
